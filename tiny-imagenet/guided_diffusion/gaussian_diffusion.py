...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if classifier_free_guidance:
            ...
            # # Set label to null token
            model_kwargs["y"][np.arange(B)] = model.num_classes - 1
            model_output_uncond = model(x, self._scale_timesteps(t), **model_kwargs)
            model_output = (1 + w) * model_output - w * model_output_uncond
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if model.predict_class:
                model_kwargs = {**model_kwargs, "return_y_pred": True}
                model_output, y_pred = model(x_t, self._scale_timesteps(t), **model_kwargs)
            else:
            ...
            if model.predict_class:
                terms["loss"] += F.cross_entropy(y_pred, model_kwargs["y"])
        else:
