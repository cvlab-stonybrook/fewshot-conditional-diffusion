...
class TrainLoop:
        ...
        if resume_checkpoint:
            ...
            if dist.get_rank() == 0:
                ...
                self.model.load_state_dict(
                    dist_util.load_state_dict(resume_checkpoint, map_location=dist_util.dev()), strict=False
                )
    ...
    def run_loop(self):
        while (
            ...
            if self.classifier_free_guidance:
                ...
                p_uncond = 0.1
                cond["y"][np.random.rand(len(batch)) < p_uncond] = self.model.num_classes - 1
    ...
    def save_samples(self):
        if dist.get_rank() == 0:
            ...
            save_path = Path(get_blob_logdir()) / f"samples_{self.step+self.resume_step}"
            save_path.mkdir(exist_ok=True, parents=True)
            self.sampling_function(self.data, self.diffusion, self.model, save_path)
...
def log_loss_dict(diffusion, ts, losses):
    for key, values in losses.items():
        logger.logkv_mean(key, values.detach().cpu().numpy().mean())
